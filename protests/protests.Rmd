...
subtitle: "Presented at the SDEG workshop on 19 September 2025"
...
Changes in the prices of staple commodities that constitute a sizable share of people’s incomes and expenditures have important welfare implications. In economies where social conditions are dire and political systems are fragile, such changes may trigger conflict.
...
# Markets and prices (WFP, FEWS, GIEWS, IMF)
...
1. we retained locally procured retail prices, reported in USD per kg.

2. we excluded series that spanned less than 10 years, had more than 10% missing observations, or contained gaps longer than four consecutive months.

3. we sequentially eliminated markets with overlapping catchment zones—the area within a 50-km radius of a centroid—with the smallest population count until no overlaps remained.
...
![](figures/markets_map_l120_r50.png)
...
![](figures/price_series_l120_r50.png)
...
# Protests and riots (ACLED)


.left-50[

![](figures/conflict_map_l120_r50.png)
...
![](figures/conflict_series_l120_r50.png)
...
$$
\begin{equation}
CONFLICT_{it}=\beta PRICE_{it} + \mu_i + \lambda_i t + \mathbf{\delta}_i'\mathbf{d}_t + \varepsilon_{it}\;~~\text{(2nd stage)}
\end{equation}
$$
...
$$
\begin{equation}
PRICE_{it}=\alpha PRICE_{t} + \kappa_i + \gamma_i t + \mathbf{\eta}_i'\mathbf{d}_t + \upsilon_{it}\;~~\text{(1st stage)}
\end{equation}
$$

where: $CONFLICT_{it}$ is a binary variable that denotes the incidence of conflict in market $i$ in period $t$; $PRICE_{it}$ is the local price and $PRICE_{t}$ is the global price, both in natural logarithms. 
...
# Negative relationship between prices and social unrest
...
# Production diversity across ethnic boundaries


.left-50[

![](figures/murdock_map_l120_r50.png)
...
![](figures/murdock_market15_l120_r50.png)

]

.right-45[

![](figures/murdock_market59_l120_r50.png)
...
# Ag dependence dampens / Ag diversity aggrevates unrest
...
| PRICE × G            |  0.625\*   |  0.603\*   |  0.875\*** |

*A* is the proportion of the area used for maize production.

*G* $=\sum_i\sum_j \pi_i\pi_j\delta_{ij}$, where $\pi_i$ and $\pi_j$ are the proportions of people residing within ethnic boundaries $i$ and $j$, and $\delta_{ij}=|a_i-a_j|$ is "distance" in the proportions of the areas used for maize production within $i$ and $j$.
...
| PRICE × G           |  0.782\**  |  0.777\**  |  0.864\*** |
| PRICE × H           |  0.039     |  0.075     | -0.041     |
| PRICE × A × H       |  0.178     |  0.091     | -0.086     |
| PRICE × G × H       | -0.583\*** | -0.643\*** |  0.025     |
...
| PRICE × G           |  0.639     |  0.608     |  0.924\*** |
| PRICE × R           |  0.009     |  0.015     |  0.010     |
| PRICE × A × R       | -0.011     | -0.028     | -0.046     |
| PRICE × G × R       | -0.093     | -0.048     | -0.210\*   |
...
| Conflict type       | Well-being | No Food    | No cash     |
|:------------------- | ----------:| ----------:| -----------:|
| PRICE               | -0.700\**  | -0.161     | -0.688\***  |
| PRICE × A           | -0.235     |  0.008     |  0.533\**   |
| PRICE × G           | -1.329\*** |  2.584     |  0.883      |

*A* is the proportion of respondents employed in ag. sector.

*G* $=\sum_i\sum_j \pi_i\pi_j\delta_{ij}$, where $\pi_i$ and $\pi_j$ are the proportions of respondents representing ethnic groups $i$ and $j$, and $\delta_{ij}=|a_i-a_j|$ is "distance" in the proportions of ag. employment across ethnic groups within a market catchment zone.
...
| Conflict type       | Unrest      | Violence   | Conflict   |
|:------------------- | -----------:| ----------:| ----------:|
| PRICE               | -0.028      | -0.026     |  0.070\*   |
| PRICE × A           |  0.069\***  |  0.047     |  0.027     |
| PRICE × G           |  1.032\***  |  0.029     | -0.187     |
